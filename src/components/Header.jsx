...
import { BsApple, BsQrCodeScan } from "react-icons/bs";
...
import {
  ...
  usePastOrders,
  useAllCarts,
  useRestaurantRecommendedProducts,
  useSearchProducts,
  useStockCheck,
  useAddAddress,
  useDeliveryFee,
...
export default function Header(props) {
  ...
  const generateTimeChunks = (startTime, endTime, date) => {
    console.log(startTime, endTime, date, "generateTimeChunks");
  ...
  // Use TanStack Query for geocoding
  let lat, lng;
  ...
  const { data: geocodeData } = useGeocode(lat, lng);
  ...
  console.log("countriesData:", getcountr);
  const {
    data,
    isLoading: favoritesLoading,
    error: favoritesError,
    isError,
  } = useFavoriteRestaurants(userId);
  console.log("favoriteRestaurantsData:", data?.data?.data?.all);
  const {
    data: getProfile,
  ...
  const { data: pastOrders, refetch: reFetchOrderHistory } = usePastOrders();
  // Initialize TanStack Query hooks for API calls
  const loginMutation = useLogin();
  const registerMutation = useRegister();
  const verifyEmailMutation = useVerifyEmail();
  const emailCheckerMutation = useEmailChecker();
  const socialRegisterMutation = useSocialRegister();
  const { searchProducts } = useSearchProductsQuery();
  const { checkStock } = useStockCheckQuery();
  ...
  const onChange2 = (e) => {
    ...
    const cleanValue = e.target.value.replace(emojiRegex, "");

    setSignUpData({ ...signUpData, [e.target.name]: cleanValue });
  };
  ...
  const [tabState, setTabState] = useState({ selected: 1 });
  ...
  const closeModal = () => {
    ...
    if (props.setLoginModal) {
      props.setLoginModal(false); // <-- This sets the login modal to false
    }
    setModalScroll(0);
  ...
  const getAddress = async () => {
    ...
    if (!isLoggedIn) {
      ...
      const countryCode = location.pathname.split("/")[1] || "pk";

      let newPath = "";

      if (addressData.city) {
        const citySlug = addressData.city.toLowerCase().replace(/\s+/g, "-");
        newPath = `/${countryCode}/${citySlug}/restaurants`;
      } else {
        newPath = `/${countryCode}/lat=${latLng.lat}&lng=${latLng.lng}/restaurants`;
      }
    ...
    if (location.pathname === resPath || location.pathname === strPath) {
      setAddressTab(2);
    } else {
      setDetailsTab(3);
    }
  ...
  const handleAddAddress = async () => {
    if (deliveryAddress.entrance === "") {
      ...
      const res = await addAddressMutation.mutateAsync(addressData);
  ...
  const deliveryFeeMutation = useDeliveryFee();
  ...
  const calculateRoute = async () => {
    ...
    await deliveryFeeMutation.mutateAsync(deliveryData);
  };
  const { data: allCartsData, refetch: refetchAllCarts } = useAllCarts(userId);
  ...
  const { data: recommendedProductsData } =
    useRestaurantRecommendedProducts(restaurantId);
  const recommendedProducts =
    recommendedProductsData?.data?.status === "1"
      ? recommendedProductsData?.data?.data?.products
      : [];
  ...
  const getMaxPhoneLength = (isoCountryCode) => {
    try {
      if (!isoCountryCode) return 15;
      ...
      console.warn("Failed to get phone length:", error);
      return 15;
    }
  };

  const validatePhoneNumber = (isoCountryCode, phoneNumber) => {
    try {
  ...
  const handlePhoneNumberChange = (value) => {
    const maxLength = getMaxPhoneLength(country.countryCode);
  ...
  const resendOTPfunc = async () => {
    ...
    await resendOTPMutation.mutateAsync(otpData);
  };
  ...
  const mergeAccounts = async (e) => {
    ...
    const res = await verifyOTPMutation.mutateAsync(otpData);
  ...
  const resendMergeOTPMutation = useResendMergeOTP();
  ...
  const handleSendMergeOtp = async () => {
    ...
    try {
      ...
      await resendMergeOTPMutation.mutateAsync(emailData);
  ...
  const signInWithGoogleFunc = async () => {
    ...
    try {
      ...
      const res = await emailCheckerMutation.mutateAsync({
        email: user?.email,
      });

      if (res?.data?.status === "1") {
        const resp = await socialRegisterMutation.mutateAsync({
  ...
  const signUpWithGoogleFunc = async (e) => {
    ...
    if (signUpData.firstName === "") {
      ...
      try {
        const res = await socialRegisterMutation.mutateAsync({
  ...
  const forgetPasswordRequestMutation = useForgetPasswordRequest();
  ...
  const forgotStepOneFunc = async (e) => {
    ...
    if (forgotData.email === "") {
      ...
      const res = await forgetPasswordRequestMutation.mutateAsync(emailData);
      if (res?.data?.status === "1") {
        setForgotData({
          ...forgotData,
  ...
  const forgetPasswordResponseMutation = useForgetPasswordResponse();
  ...
  const forgotStepThreeFunc = async (e) => {
    ...
    if (forgotData.password === "") {
      ...
      const res = await forgetPasswordResponseMutation.mutateAsync(
        passwordData
      );
      if (res?.data?.status === "1") {
        setForgotStep(4);
  ...
  const searchFunction = async () => {
    ...
    try {
      const res = await searchProducts(termToSearch, headerSearch.openFilter);
  ...
  const handleLogoClick = () => {
    ...
    const restaurantPath = `/${countryCode}/${cityName}/restaurants`;
    const storePath = `/${countryCode}/${cityName}/store`;
    ...
    if (currentPath === restaurantPath || currentPath === storePath) {
      navigate("/"); // Go to home from restaurants/store
    } else {
      navigate(restaurantPath); // Default fallback to restaurant listing
    }
  };
  ...
  const cartItems = cartItem[resKey] || [];
  ...
  const delGroup = async () => {
    ...
    const res = await deleteGroupMutation.mutateAsync(groupData);
    if (res?.data?.status === "1") {
  ...
  const removeMember = async (orderId, userId) => {
    ...
    const res = await removeMemberMutation.mutateAsync(memberData);
    if (res?.data?.status === "1") {
  ...
  const leaveGroup = async (orderId, userId) => {
    ...
    const res = await leaveGroupMutation.mutateAsync(groupData);
    if (res?.data?.status === "1") {
      success_toaster("Group left successfully");
      setGroupDrawer(false);
      setGroup({ ...group, show: 0 });
  ...
  const addCart = async () => {
    const userId = localStorage.getItem("userId");
    const restaurantId = localStorage.getItem("resId");
    const fullCart = JSON.parse(localStorage.getItem("cartItem")) || {};
    const itemsForRestaurant = fullCart[`id_${restaurantId}`] || [];
    localStorage.setItem("cart", JSON.stringify(itemsForRestaurant));
    const transformedItems = itemsForRestaurant.map((item) => {
      return {
        quantity: item.quantity.toString(),
        unitPrice: item.unitPrice.toFixed(2), // convert to string with decimals
        RPLinkId: item.RPLinkId.toString(),
        name: item.name,
        image: item.image,
        uniqueId: item.uniqueId || "initialProduct", // fallback if missing
        menuCatId: item.menuCatId || null, // add if available, else null
        appliedDiscountBannerId: null,
        bannersAppliedOnProduct: [],
        appliedFreeDeliveryBannerId: null,
        appliedBogoBannerId: null,
        addOns: (item.addOns || []).map((addOn) => ({
          total: parseFloat(addOn.total).toFixed(2),
          quantity: addOn.quantity.toString(),
          collectionId: addOn.collectionId.toString(),
          collectionName: addOn.collectionName || "", // optional
    ...
    // Call your API
    try {
      await addCartMutation.mutateAsync(payload);
    } catch (error) {
  ...
  const handleMoreItem = (cartId) => {
    const clickedCart = allCarts.find((cart) => cart.id === cartId);
    if (!clickedCart) return;

    const resId = clickedCart.restaurant.id;
    const cartData = JSON.parse(localStorage.getItem("cartItem")) || {};
    const resKey = `id_${resId}`;
    if (!cartData[resKey] || cartData[resKey].length === 0) {
      let products = [];
      try {
        products = JSON.parse(clickedCart.data);
      } catch (e) {
        console.error("Failed to parse clickedCart data", e);
      }

      cartData[resKey] = products;
      localStorage.setItem("cartItem", JSON.stringify(cartData));
      setCartLength(cartData.length);
    } else {
      console.log(
        `Cart already exists for restaurant ${resId}, not overwriting.`
      );
    }
  ...
  const handleOrderAgain = (order) => {
    const resId = order.restaurant.id;
    const resKey = `id_${resId}`;
    const cartData = JSON.parse(localStorage.getItem("cartItem")) || {};

    let products = [];
    try {
      const parsedOrderAgain = JSON.parse(order.orderAgain);
      products = parsedOrderAgain.products || [];
    } catch (e) {
      console.error("Failed to parse order.orderAgain", e);
    }

    // Store cart data
    cartData[resKey] = products;
    localStorage.setItem("cartItem", JSON.stringify(cartData));
    setCartLength(Object.keys(cartData).length); // safer than cartData.length
    ...
    const zone = restaurant?.zoneRestaurant?.zone;
    const city = zone?.city?.name || restaurant?.city;
    const country = zone?.city?.country?.shortName || "pk";

    const businessSlug =
      restaurant.businessName.toLowerCase().split(" ").join("-") +
      "-" +
      (restaurant.businessType == "1" ? "res" : "store") +
      "-" +
      restaurant.id;
  ...
  const handleDeleteCart = async (cartId) => {
    ...
    const res = await deleteCartMutation.mutateAsync(cartData);
    if (res?.data?.status === "1") {
      refetchAllCarts();
    }
  };
  const lockGroupOrderMutation = useLockGroupOrder();
  ...
  const lockGroup = async (orderId, isLocked) => {
    ...
    const res = await lockGroupOrderMutation.mutateAsync(lockData);
    if (res?.data?.status === "1") {
      setGroupDrawer(false);
      setGroup({ ...group, show: 0 });
    }
  ...
  const handleSubstitution = (RPLinkId, action, replaceWithValue) => {
    if (cartItems.length > 0) {
      ...
      cartItemUpdated[resKey] = updatedCartItems;
      localStorage.setItem("cartItem", JSON.stringify(cartItemUpdated));
    ...
    if (
      ...
      const itemsToCheck = items.filter((item) => {
        const cartItem = cartItems.find(
          (cartItem) => cartItem.RPLinkId === item.id
        );
        return cartItem?.isUnlimited === false;
      });
      const res = await checkStock(itemsToCheck);
  ...
  const [logoLoadedMap, setLogoLoadedMap] = useState({});
  const handleLogoLoad = (id) => {
    setLogoLoadedMap((prev) => ({ ...prev, [id]: true }));
  };

  const handleCartFromHome = async (cartId) => {
    const clickedCart = allCarts.find((cart) => cart.id === cartId);
    localStorage.setItem("resId", JSON.stringify(clickedCart?.restaurant?.id));
    addCart();
    if (activeResData?.smallOrderFee) {
      if (Number(totalPrice) < Number(activeResData?.minOrderAmount)) {
        const difference =
    ...
    if (
      ...
      // Filter out unlimited products before sending to stock check
      const itemsToCheck = items.filter((item) => {
        const cartItem = cartItems.find(
          (cartItem) => cartItem.RPLinkId === item.id
        );
        return cartItem?.isUnlimited === false; // Only check stock for non-unlimited products
      });
  ...
  const getCallingCode = (countryCode) => {
    try {
      ...
      const cleanedCode = countryCode.startsWith("+")
        ? countryCode.slice(1)
        : countryCode;
      const code = getCountryCallingCode(cleanedCode.toUpperCase());
      return `+${code}`;
    } catch {
      return "";
    }
  ...
  useEffect(() => {
    ...
    // ✅ Auto-switch to Delivery when only Delivery is allowed
    if (!disableDelivery && disablePickup && deliveryData.how !== 1) {
      setDeliveryData((prev) => ({
        ...prev,
        how: 1,
        ...(disableStandard && !disableSchedule ? { type: 2, when: 2 } : {}),
      }));
      if (disableStandard && !disableSchedule)
        localStorage.setItem("when", "2");
      return; // prevent running the rest in this pass
    }
    if (
      (deliveryData.type === 2 || deliveryData.when === 2) &&
      !disableStandard &&
      disableSchedule
    ) {
      setDeliveryData((prev) => ({
    ...
    if (disableStandard && !disableSchedule) {
      if (deliveryData.type !== 2)
        setDeliveryData((prev) => ({ ...prev, type: 2 }));
      if (deliveryData.when !== 2) {
        setDeliveryData((prev) => ({ ...prev, when: 2 }));
        localStorage.setItem("when", "2");
      }
      // ensure `how` is set to a valid mode when it's unset/invalid
      if (deliveryData.how !== 1 && deliveryData.how !== 2) {
        setDeliveryData((prev) => ({ ...prev, how: !disableDelivery ? 1 : 2 }));
      }
    }
                  ...
                  >
                    ...
                    <FloatingLabelInput
                      ...
                      disabled={["google", "apple", "facebook"].includes(
                        signUpData.signedFrom?.toLowerCase()
                      )}
                    />
                    ...
                    <div className="flex gap-x-1">
                      <CountrySelect
                        labels={en}
                        onChange={(isoCode) =>
                          setSignUpData({ ...signUpData, countryCode: isoCode })
                        }
                      />
                      <FloatingLabelInput
                        id="phoneNum"
                ...
                <>
                  <div className="space-y-3">
                    ...
                    <form onSubmit={otpFunc}>
                      ...
                      <button
                        type="submit"
                        className="mt-2 py-[14px] px-5 w-full bg-theme-red hover:bg-opacity-95 text-white font-bold shadow-md text-base rounded-lg border border-theme-red font-sf"
                      >
                        Verify OTP
                      </button>
                    </form>
              ...
              {addressTab === 0 ? (
                <div className="">
                  <div className="space-y-4 px-4 ">
                    ...
                    {userAddresses?.data?.data?.addressList
                      ?.filter(
                        (fil) =>
                          fil.AddressType &&
                ...
                <div className="">
                  <div className="space-y-4  px-4">
                    ...
                    {getcountr?.data?.countries.map((countr, index) => (
                      <div key={index} className="">
                        <div className="flex items-center gap-x-4 w-full pb-4">
                          <button className="flex justify-center items-center text-end outline-none w-8 h-12 flex-shrink-0 ">
                            <ReactCountryFlag
                                ...
                                <div className="text-base font-semibold">
                                  {deliveryData.how === 1 ? (
                                    deliveryAddress?.id !== "" ? (
                                      ...
                                      <>{t("Please add a delivery address")}</>
                                    )
                                  ) : (
                                    <></>
                                  )}
                      ...
                      {data?.data?.data?.all?.length > 0 ? (
                        <>
                          <div className="swiper-container w-full">
                            ...
                            >
                              {data?.data?.data?.all.map((res, index) => (
          ...
          >
            <div className="space-y-6 font-sf px-4 mb-28">
              ...
              >
                {props.cart && props.home && !props.rest ? (
                                  ...
                                  <>
                                    <button
                                      onClick={() => {
                                        ...
                                        if (quan > 1) {
                                          // Find item in restaurant cart
                                          const incCart = restaurantCart.find(
                                            (ele) =>
                                          ...
                                          const existingIndex =
                                            restaurantCart.findIndex(
                                              (ele) =>
                                                ele.RPLinkId === cartI.RPLinkId
                                            );
                                          if (existingIndex !== -1) {
                                            restaurantCart.splice(
                                              existingIndex,
                                              1
                                            );
                                          }
                                        ...
                                        // Save updated restaurant cart back in cartItems object
                                        cartItems[resKey] = restaurantCart;
                                        localStorage.setItem(
                                          "cartItem",
                                    ...
                                    <button
                                      onClick={() => {
                                        ...
                                        if (existingIndex !== -1) {
                                          ...
                                          cartData[resKey] = restaurantCart;
                                          localStorage.setItem(
                                            "cartItem",
                ...
                ) : props.home || props?.discovery ? (
                  // Render this block if props.home is true
                  <>
                    <div className="p-1 rounded-full bg-theme-gray-4 grid grid-cols-2 relative overflow-hidden">
                      <LayoutGroup id="header-cart-order-tabs">
                        <button
                          onClick={() => {
                        ...
                        >
                          {tabState.selected === 1 && (
                            <motion.span
                              layoutId="cartOrderTabHighlight"
                    ...
                    <div className="flex flex-col space-y-4 mt-4 font-sf">
                      {tabState.selected === 1 ? (
                        allCarts.length === 0 ? (
                          <div className="flex flex-col items-center justify-center h-3/5 mt-24">
                            <img
                            ...
                            <p className="mt-2 text-gray-500 text-sm font-sf text-center px-4">
                              When you add items from a restaurant or store,
                              your order will be shown right here. You can make
                          ...
                          allCarts.map((cart) => {
                            ...
                            const subtotal = products.reduce((acc, p) => {
                              ...
                              const addOnsTotal = (p.addOns || []).reduce(
                                (a, addOn) => {
                                  return (
                                    a +
                              ...
                              return acc + (price * quantity + addOnsTotal);
                            }, 0);

                            return (
                              <div
                                key={cart.id}
                                className="border-[2px] border-theme-gray-12 rounded-xl pb-4 mb-2"
                              >
                                <div className="flex justify-between items-center p-4">
                                  <div className="flex space-x-4">
                                    <div className="w-[56px] h-[56px] rounded-lg overflow-hidden border">
                                      <Skeleton
                                        isLoaded={logoLoadedMap[cart.id]}
                                        height="100%"
                                        width="100%"
                                        borderRadius="lg"
                                        startColor="gray.300"
                                  ...
                                  <div
                                    className="flex justify-center items-center bg-theme-gray-12 rounded-full p-1 cursor-pointer"
                                ...
                                <div className="w-full border border-dashed border-gray-12"></div>

                                <div className="flex items-center space-x-2 mt-4 px-4">
                                  {products.map((product, idx) => (
                                    <div
                                      key={idx}
                                      className="w-16 h-10 rounded-lg bg-theme-gray-4 overflow-hidden border"
                                    >
                                      <img
                                ...
                                <div className="px-4 space-x-4 flex justify-between item-center mt-4">
                                  <button
                                    onClick={() => handleMoreItem(cart.id)}
                                    className="w-full py-[12px] rounded-md font-medium font-sf bg-theme-red bg-opacity-20 text-theme-red"
                                  >
                                    Add more items
                                  </button>
                                  <button
                                    className="w-full py-[12px] rounded-md font-medium font-sf bg-theme-red text-white"
                                    onClick={() => handleCartFromHome(cart.id)}
                                  >
                                    Go to checkout
                                  </button>
                                </div>
                        ...
                        <div className="text-center text-sm text-theme-black-2 mt-4">
                          {pastOrders?.data?.data?.orders?.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-3/5 mt-24 space-y-8">
                              <img
                                src="/images/noPastOrder.png"
                                alt="Empty cart"
                                className="w-full max-w-96 h-full object-contain"
                              />
                              <h2 className="text-[28px] font-omnes font-bold">
                                No Past Orders
                              </h2>
                            ...
                            pastOrders?.data?.data?.orders?.map((order) => (
                              <div
                                key={order.id}
                                className="border-[2px] border-theme-gray-12 rounded-xl pb-4 mb-2"
                              >
                                <div className="flex justify-between items-center p-4">
                                  <div className="flex space-x-4">
                                    <div className="w-[56px] h-[56px] rounded-lg overflow-hidden border">
                                      <img
                                        src={`${BASE_URL}${order.restaurant?.image}`}
                                ...
                                {/* Items Preview */}
                                <div className="flex items-center space-x-2 mt-4 px-4 overflow-x-auto">
                                  {(() => {
                                    let products = [];
                                    try {
                                ...
                                <div className="px-4 mt-4 text-left">
                                  <h2 className="font-semibold text-theme-black-2 text-sm">
                                    Total Paid: {order.total}{" "}
                                    {order.restaurant?.zoneRestaurant?.zone
                                      ?.zoneDetail?.currencyUnit?.symbol ||
                                      "CHF"}
                                  </h2>
                                </div>

                                <div className="px-4 space-x-4 flex justify-between items-center mt-4">
                                  <button
                                    className="w-full py-[12px] rounded-md font-medium font-sf bg-theme-red bg-opacity-20 text-theme-red"
                                    onClick={() =>
                                      navigateToTimeline(order?.id)
                                    }
                                  >
                                    View Details
                                  </button>
                          ...
                          <div className="h-3/5 overflow-y-auto   ">
                            ...
                            <div>
                              {recommendedProducts.length > 0 && (
                                <>
                                  <h2 className="font-semibold text-theme-black-2 font-omnes text-2xl mt-10">
                                    Recommended for you
                                  </h2>
                                  <div className="grid grid-cols-2 gap-4 auto-rows-auto overflow-hidden p-1 mt-4">
                                    {recommendedProducts.map((prod) => {
                                      const cartItemsForProduct =
                                        existingCartItems?.filter(
                                          (item) =>
                                            item?.RPLinkId === prod.RPLinkId
                                        );
                                      const isInCart =
                                        cartItemsForProduct?.length > 0;
                                      const hasAddOns =
                                        prod?.addOnArr &&
                                        prod?.addOnArr.length > 0;
                                      ...
                                      return (
                                        <StoreCard
                                          ...
                                          addCart={() => {
                                            if (isInCart) {
                                              setDrawerCart(false);
                                              props?.setAddDiffProductOpMessage(
                                          ...
                                          addWish={() =>
                                            addToWishlistFunc(prod?.r_pId)
                                          }
                                          hasAddOns={hasAddOns}
                                          addOns={prod?.addOnArr}
                                          directAddToCart={(newQuantity) => {
                                            if (newQuantity === 0) {
                                              const cartItem =
                                                JSON.parse(
                  ...
                  <>
                    ...
                    <button
                      className=" bg-black text-white rounded-lg h-[54px] my-5 w-[94%] absolute z-[100] bottom-0 left-1/2 transform -translate-x-1/2 min-h-14 "
                      onClick={() => {
            ...
            <div className=" sticky  bottom-0   bg-white pb-2 z-50">
              {drawerMsg === false && !props?.home && !props?.discovery ? (
                <>
                  ...
                  <div className="w-10 h-10 rounded-full overflow-hidden">
                    {/* <img
                      ...
                      onError={(e) => {
                        ...
                        e.target.src = "/images/userIcon.jpeg";
                      }}
                      alt=""
                    /> */}
                    <img
            ...
            <div className="z-20 font-sf max-w-[1200px] w-full mx-auto">
              ...
              >
                ...
                <div className="flex">
                  ...
                  {localStorage.getItem("loginStatus") !== "true" && (
                    <div className="flex gap-x-2 md:h-[40px] items-center">
                        ...
                        <Menu>
                          ...
                          >
                            <span className="flex items-center justify-center w-full h-full">
                              <GrLanguage size={20} color="#202125" />
                            </span>
                          </MenuButton>
                          <MenuList>
                      ...
                      {allCarts?.length > 0 && userId && (
                        ...
                        >
                          ...
                          <div className="absolute top-0 right-0 w-4 h-4 flex justify-center items-center bg-theme-red text-white text-xs rounded-fullest">
                            {allCarts?.reduce((accumulator, currentItem) => {
                ...
                {pending === "data" ? (
                  (searchResult?.data?.list?.length > 0 ||
                    ...
                    <div className="w-full max-w-[708px] mx-auto">
                      ...
                      {searchResult?.data?.list.length === 0 && (
                        <p className="text-gray-500 px-2 mt-4 mb-14">
                            ...
                            <div
                              ...
                              onClick={() => {
                                resDetails(
                                  item?.id,
                                  item?.city,
                                  item?.countryShortName?.toLowerCase(),
                        ...
                        {productList?.length > 0 && (
                          <p
                            onClick={() => {
                              ...
                              if (!res.includes(trimmedSearch)) {
                                res.push(trimmedSearch);
                                localStorage.setItem(
                                  "recentSearch",
                                  JSON.stringify(res)
                                );
                      ...
                      {productList?.length === 0 && (
                        <p className="text-gray-500 px-2">No results found.</p>
                      )}
