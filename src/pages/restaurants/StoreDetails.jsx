...
import StoreSideMenu from "../../components/StoreSideMenu";
...
export default function StoreDetails() {
  ...
  const copyToClipboard = (text) => {
    navigator.clipboard.writeText(`localhost:5173${text}`);
  ...
  const handleGroupOrder = (e) => {
    if (e.target.name === "icon") {
      setGroupOrder({ ...groupOrder, [e.target.name]: e.target.files[0] });
    } else {
      setGroupOrder({ ...groupOrder, [e.target.name]: e.target.value });
    }
  };
  ...
  const handelSelectDate = (date) => {
    ...
    if (selectedDay) {
      const startTime = dayjs(selectedDay.startAt, "HH:mm");
      const endTime = dayjs(selectedDay.endAt, "HH:mm");
  ...
  const productDetails = (rpId, name, r_mcId, material) => {
    ...
    if (prod) {
      ...
      navigate(
        `/${countryCode}/${cityName}/stores/${slug}/${formattedName}-${rpId}`,
        { replace: true }
      );
    }
  };
  ...
  const calculateUpcomingDays = () => {
    const upcomingDaysArray = [];
    for (let i = 0; i < 5; i++) {
      const nextDay = new Date();
      nextDay.setDate(today.getDate() + i + 1);
      const nextDayDate = nextDay.getDate();
      const nextDayDay = daysOfWeek[nextDay.getDay()];
      upcomingDaysArray.push({ date: nextDayDate, day: nextDayDay });
    }
    // setUpcomingDays(upcomingDaysArray);
  };
  ...
  const calculateRoute = async () => {
    ...
    setGroupOrder({
      ...
      building: place.name,
      VAT: data?.data?.VAT,
      service_charges: data?.data?.service_charges,
    });
  ...
  useEffect(() => {
    if (location.pathname !== "/store-details/detail") {
    ...
    <>
          ...
          <ModalBody p={0} borderRadius="20px">
            {productInfo ? (
              <div
                ...
                className="md:h-screen-minus-5vh ultraLargeDesktop:h-screen-minus-40vh h-screen-minus-18vh  overflow-auto font-sf space-y-5 px-4 pt-10 setScroll"
              >
                  ...
                  <div>
                    ...
                    <div className="border-t-4 border-b-4 border-theme-black-2 mt-2">
                      <div className="flex flex-wrap justify-between items-center [&>div]:border-b [&>div]:items-center [&>div]:py-1 [&>div]:text-[#202125A3]">
                        ...
                        <div className="flex justify-between w-full">
                          <p>Fiber</p> {modalData?.nutritionalInfo?.fiber ?? ""}
                        </div>
                ...
                <div className=" mt-2">
                  <div className="px-4">
                    <div>
                      ...
                      <h5 className="font-switzer text-greenColor font-semibold line-through mt-2">
                                ...
                                >
                                  <div className="flex  gap-x-1">
                                    ...
                                    >
                                      ...
                                      selectedQuantity > 0 ? (
                                        <>
                                          <span className="text-theme-green-2 font-semibold ">{`${selectedQuantity}x`}</span>
                                          {` ${addOn?.name}`}
                                        </>
                                      ) : (
                                        addOn?.name
                    ...
                    <div className="flex flex-col w-full">
                      <div className="px-5 py-5 flex justify-center items-center  gap-3 w-full">
                        <div className="shadow-smButtonShadow  w-40 h-14 rounded-full flex items-center justify-around text-[#707175] bg-white">
                          ...
                          <span className="text-lg font-switzer">
                            {modalData.quantity}
                          </span>
                          <button
                            onClick={() =>
                          ...
                          onClick={() => {
                            ...
                            if (!allOptionsDisabled) {
                              if (modalData.quantity > 0) {
                                ...
                                navigate(
                                  `/${countryCode}/${cityName}/stores/${slug}`
                                );
                              } else {
                              ...
                              info_toaster("Store is closed right now!");
                            }
                          }}
                          bgColor="black"
                          display="flex"
                        ...
                        >
                          <div className="text-md font-switzer font-semibold">
                            {modalData.quantity > 0
                              ? existingCartItems?.find(
                                  (ele) => ele?.RPLinkId === modalData.r_pId
                                )
              ...
              <>
                <div className="w-full font-tt">
                  ...
                  <div className="flex gap-x-3 justify-end p-5">
                    <button
                      className="bg-black rounded-md py-2 text-white w-1/2"
                    ...
                    <button
                      ...
                      onClick={() =>
                        navigate(`/${countryCode}/group-order/${orderId}/venue`)
                      }
                    >
                      Continue
                    </button>
      ...
      >
        ...
        >
          ...
          >
            <section className="max-h-[calc(100vh-200px)] ultraLargeDesktop:h-screen-minus-40vh h-auto overflow-auto px-6  space-y-4">
      ...
      >
        ...
        <ModalContent>
          {step === 1 ? (
            <>
              <ModalHeader>
                <h2 className="text-xl text-center font-switzer">
                  Group Order
                </h2>
              </ModalHeader>
              <ModalCloseButton />
              <ModalBody>
                <section className="font-sf">
                  <div className="bg-[#F4F4F4] px-3 py-1 rounded-md">
                    <h4 className="text-xl text-black font-switzer font-bold capitalize">
                      Order as a group
                    </h4>
                    <p>
                      Invite friends and family to order together. Here's how it
                      works:
                    </p>
                  </div>

                  <div className="">
                    <ul className="px-4 py-1 list-decimal">
                      <li>
                        Create a group and share the link with your guests.
                      </li>
                      <li>
                        When everyone has chosen their items, confirm the order
                        and a courier will deliver it!
                      </li>
                    </ul>
                    <div className="my-4">
                      <img
                        className="w-72 object-cover m-auto"
                        src="/images/restaurants/group.webp"
                        alt=""
                      />
                    </div>
                  </div>
              ...
              <ModalFooter>
                <div className="text-center w-full ">
                  <button
                    onClick={() => setSteps(2)}
                    className="text-white bg-[#E13743] px-4 py-2 rounded-sm"
                  >
                    Get Started
                  </button>
                </div>
            ...
            <>
              <ModalHeader>
                <h2 className="text-xl text-center">Group Order</h2>
              </ModalHeader>
              <ModalCloseButton />
              <ModalBody>
                <section>
                  <div>
                    ...
                    <div className="flex flex-col">
                      <label
                        className="text-black text-opacity-50 my-2"
                        htmlFor="groupName"
                      >
                        Group Name
                      </label>
                      <input
                        ...
                        onChange={handleGroupOrder}
                        name="groupName"
                        className="py-2.5 px-4 border border-black border-opacity-20 w-full rounded focus:outline-none font-normal md:text-base text-sm"
                        type="text"
                        id="groupName"
                      />
                    </div>
                  ...
                  <div>
                    <div className="flex flex-col gap-x-3">
                      <label htmlFor="icon">Uplaod Group Icon</label>
                      <input
                        type="file"
                        onChange={handleGroupOrder}
                        name="icon"
                    ...
                    <div className="flex justify-between items-center my-4">
                      {deliveryTypes?.data?.data?.deliveryTypes.map(
                        (delivery, index) => (
                          <label
                            className="cursor-pointer"
                            htmlFor={`${delivery.name}${index}`}
                          >
                            {delivery.name}
                            <input
                              value={delivery?.id}
                    ...
                    <div>
                      <label className="text-lg my-3">Where to ?</label>
                      <Autocomplete
                        onLoad={(autocomplete) =>
                    ...
                    <div>
                      <h6 className="text-lg my-3">When ?</h6>
                      <div>
            ...
            <>
              <ModalHeader>
                <h2 className="text-sm  text-black text-opacity-50">
                  Maco and friends
                </h2>
                <h2 className="text-xl font-semibold">Invite your guests</h2>
              </ModalHeader>
              <ModalCloseButton />
              <ModalBody>
                <section>
                  <div>
                    <p className="text-black text-opacity-50">
                      Share the link below to invite your guests. When they've
                      joined and added their choices, send your order! We'll
                      take care of the rest.
                    </p>
                    <div>
                      <b>Please note:</b>
                      <p>
                        As the host, you'll be charged the full amount for the
                        group order.
                      </p>
                    </div>
                    <div className="bg-[#FFF6E2] rounded-md px-3 py-1">
                      <p>Group name friends Tap to copy shareable link</p>
                    </div>
                    <div className="my-4">
                      <img
                        className="w-72 object-cover m-auto"
        ...
        <ModalContent backgroundColor="transparent">
          ...
          >
            <div className="w-full px-4 py-9 flex flex-col justify-center items-center gap-3">
              <h2 className="font-omnes text-[28px] font-bold">
                Continue Order?
              </h2>
      ...
      <section className="font-sf">
        <section className="relative border-b-[1px] border-b-black border-opacity-10">
          ...
          <div className=" custom-max-width w-[92%] lg:w-[96.3%] largeDesktop:w-[95%] extraLargeDesktop:w-5/6 mx-auto pt-10 pb-4 md:py-4 md:flex lg:flex-row sm:flex-col md:flex-col-reverse lg:justify-between lg:items-center items-end gap-y-3 relative bg-white space-y-2 md:space-y-0">
            <div className=" md:w-28 w-24 md:h-28 h-24 absolute lg:left-0 left-0  md:left-0 largeDesktop:left-6 -top-16 md:-top-20 border-4 bg-white border-white rounded-lg shadow-md ">
              <img
            ...
            <div className=" flex md:flex-row flex-col items-start md:items-center xl:gap-x-3 gap-x-2 space-y-2 md:space-y-0">
        ...
        <section className="md:grid grid-cols-12 gap-x-4 custom-max-width md:px-[30px]  mx-auto mt-2 md:mt-10 mb-10  space-y-5 md:space-y-0 ">
          {md ? (
            <div className="md:pe-7  col-span-5 xl:col-span-3 sticky top-28 lg:top-20  space-y-3 md:space-y-6 md:h-screen extraLargeDesktop:h-screen md:overflow-y-scroll z-40 bg-white pt-5 md:pt-0 custom-scrollbar2">
              <div className="mx-3 md:mr-4 md:mx-0 sticky top-0 bg-white">
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => {
                  ...
                  className="py-2 px-5 rounded-3xl bg-theme-gray-8 focus:outline-none font-normal text-base w-full placeholder:text-sm pl-10"
                  placeholder={`Search in  ${data?.data?.name}`}
                />
                <IoSearch
                  size={20}
                  className={`absolute top-1/2 -translate-y-1/2 left-4 text-theme-black-2 pointer-events-none`}
                />
              </div>
              <div
                className={`relative w-full flex md:flex-col overflow-x-auto store-cat-section px-2.5 md:px-0 ${
                  addShadow ? "shadow-lg md:shadow-none" : ""
                }`}
              >
                {data?.data?.menuCategories?.map((menu, index) => (
                  <>
                    <div
                      onClick={() => scrollToCategory(menu.name)}
                      key={index}
                      className={`space-y-1 md:mr-4 cursor-pointer   rounded-lg hover:bg-[#f6f6f6] ${
                        activeMenu === `menu-${menu.name}`
                          ? "text-[#FFAE00] bg-[#FFAE0014]"
                          : "text-theme-black-2"
                      }`}
                    >
                      <StoreSideMenu
                        img={`${BASE_URL}${menu?.iconImage}`}
                        title={menu?.name}
                      />
                    </div>
                  </>
            ...
            <div className="cat-section py-3 shadow-md w-full">
          ...
          <div className="md:w-full w-[92%] sm:w-[96%] col-span-7 xl:col-span-9 space-y-5 sticky mx-auto">
            {searchQuery.trim() === "" &&
            ...
            {getSearchedResults().length > 0 ? (
              <>
                {getSearchedResults()?.map((menu, index) => (
                  <>
                    <div
                      className="flex items-center gap-x-4  section"
                      key={index}
                      id={`menu-${menu?.name}`}
                      ref={(el) => (categoryRefs.current[index] = el)}
                    >
                      <h5 className="capitalize font-semibold text-[24px] font-omnes ">
                        {menu?.name}
                      </h5>
                    </div>

                    <div className="grid grid-cols-2 xl:grid-cols-4 smallDesktop:grid-cols-3 gap-4 ">
                      {menu?.products?.map((prod, key) => {
                        ...
                        const hasAddOns =
                          prod?.addOnArr && prod?.addOnArr.length > 0;
                        return (
                          <LazyLoadComponent key={prod.RPLinkId || key}>
                            <StoreCard
                              quantity={
                                existingCartItems?.find(
                              ...
                              addWish={() => addToWishlistFunc(prod?.r_pId)}
                              cartItems={cartItemsForProduct}
                              hasAddOns={hasAddOns}
                              ...
                              directAddToCart={(newQuantity) => {
                                if (newQuantity === 0) {
                                  ...
                                  const resKey = `id_${resId}`;
                                  const restaurantCart = cartItem[resKey] || [];
                                  const updatedCart = restaurantCart.filter(
                                    (item) => item.RPLinkId !== prod.RPLinkId
                                  );
                ...
                <div className="mx-auto w-full text-center pb-10">
                  <figure>
                    <img
                      ...
                      src="/images/restaurant-details/no-data.webp"
                      alt=""
                    />
                  </figure>
                  {searchQuery.trim() !== "" && (
      ...
      <Footer width="w-[94%] lg:w-[95%] largeDesktop:w-[95%] extraLargeDesktop:w-5/6 mx-auto " />
    </>
  );
}
